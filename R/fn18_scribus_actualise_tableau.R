#' fn18_scribus_actualise_tableau
#'
...
#' @export
fn18_scribus_actualise_tableau <- function(data = ls_tab) {
  # Modification des tableaux --------
  #

  purrr::map(
    data,
    ~ .x %>% dplyr::mutate_if(.predicate = is.factor, .funs = as.character)
  ) -> ls_tab

  fn_tableau_transforme_tabw_to_tabl <- function(x = ls_tab[[1]]) {
    x -> df0
    nrow(df0) -> nblgn
    ncol(df0) -> nbcol
    tidyr::expand_grid(
  ...
  purrr::map(
    ls_tab_contenu,
    ~ .x %>%
      dplyr::mutate(txt_new = utf8::as_utf8(txt_new))
  ) -> ls_tab_contenu
  ...
  purrr::pwalk(
    ...
    .f = fn19_scribus_modifie_tableau
  )
}
